...
class TestNfvVnf(unittest.TestCase):
    ...
    def __test_vnsf_running_list(self, url=None, schema=None,
                                 exp_code=None, exp_out=None):
        if url is None:
            url = "{0}{1}".format(self.utils.url, self.vnsf_running_list)
        if exp_code is None:
            exp_code = 200
        if exp_out is None:
    ...
    def test_mocked_vnsf_running_list(self):
        ...
        exp_out = vnf_s.get_vnfr_running(**{"mock": True})
        schema = vnfs_m().get_vnfr_running_schema()
        self.__test_vnsf_running_list(url, schema, exp_code, exp_out)
